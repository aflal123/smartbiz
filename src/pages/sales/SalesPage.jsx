...
import {
  ...
  ShoppingCart, Person, Print, Download,
  Edit
...
const generatePDF = (sale, download = false) => {
  ...
  doc.text(`Rs. ${Number(sale.totalAmount).toLocaleString()}`, 195, finalY + 8, { align: 'right' });

  doc.text('Discount:', totalsX, finalY + 16);
  doc.text(`- Rs. ${Number(sale.discount).toLocaleString()}`, 195, finalY + 16, { align: 'right' });
  ...
  doc.text(`Rs. ${Number(sale.finalAmount).toLocaleString()}`, 195, finalY + 30, { align: 'right' });

  doc.setTextColor(0, 0, 0);
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(10);
  doc.text('Amount Paid:', totalsX, finalY + 40);
  doc.text(`Rs. ${Number(sale.amountPaid).toLocaleString()}`, 195, finalY + 40, { align: 'right' });

  doc.text('Change:', totalsX, finalY + 48);
  doc.text(`Rs. ${Number(sale.changeAmount).toLocaleString()}`, 195, finalY + 48, { align: 'right' });

  // ── FOOTER ────────────────────────────────────────
  doc.setFontSize(9);
  doc.setTextColor(150, 150, 150);
  doc.text('Thank you for your business!', 105, 280, { align: 'center' });
  doc.text('Generated by SmartBiz — AI-Powered Business Management', 105, 286, { align: 'center' });
  ...
  return (
    ...
    >
      <DialogTitle sx={{
        ...
        <Typography variant="h6" fontWeight={600}>Create New Sale 🛒</Typography>
      ...
      <DialogContent dividers sx={{ maxHeight: '70vh', overflowY: 'auto' }}>
        <Grid container spacing={2} sx={{ pt: 1 }}>
          ...
          <Grid item xs={12}>
            <Box sx={{ border: '1px solid #e2e8f0', borderRadius: 2, overflow: 'hidden' }}>
              <Box sx={{
                ...
                <Typography variant="subtitle2" fontWeight={600}>Sale Items</Typography>
          ...
          <Grid item xs={12}>
            <Box sx={{ bgcolor: '#f0f4ff', borderRadius: 2, p: 2, border: '1px solid #c7d7fd' }}>
  ...
  return (
    ...
    >
      <DialogTitle sx={{
        ...
        <Typography variant="h6" fontWeight={600}>{sale.invoiceNumber}</Typography>
        <IconButton onClick={onClose}><Close /></IconButton>
      </DialogTitle>
      ...
      <DialogContent dividers>
        ...
        {/* Customer */}
        <Box sx={{ mb: 2 }}>
          <Typography variant="body2" color="text.secondary" mb={0.5}>Customer</Typography>
          <Typography variant="body1" fontWeight={600}>
            {sale.customer?.name || 'Walk-in Customer'}
          </Typography>
          {sale.customer?.phone && (
            <Typography variant="body2" color="text.secondary">{sale.customer.phone}</Typography>
          )}
        </Box>
        ...
        {/* Items */}
        <Typography variant="subtitle2" fontWeight={600} mb={1}>Items Purchased</Typography>
        <Table size="small">
          <TableHead>
            <TableRow sx={{ bgcolor: '#f8fafc' }}>
              <TableCell>Product</TableCell>
          ...
          <TableBody>
            {sale.items?.map((item) => (
              <TableRow key={item.id}>
                ...
                <TableCell align="right">Rs. {Number(item.subtotal).toLocaleString()}</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
        ...
        <Box sx={{ mt: 2, display: 'flex', gap: 1, flexWrap: 'wrap', alignItems: 'center' }}>
          <Chip label={sale.paymentMethod?.toUpperCase()} size="small" color="primary" variant="outlined" />
          <Chip
            label={sale.status?.toUpperCase()} size="small"
            color={sale.status === 'paid' ? 'success' : sale.status === 'cancelled' ? 'error' : 'warning'}
          />
        ...
        {sale.notes && (
          <Alert severity="info" sx={{ mt: 2, borderRadius: 2 }}>{sale.notes}</Alert>
        )}
...
const SalesPage = () => {
  ...
  const columns = [
    ...
    {
      field: 'finalAmount', headerName: 'Amount', width: 140,
      renderCell: (params) => (
    ...
    },
    {
      ...
      renderCell: (params) => (
        <Chip label={params.value?.toUpperCase()} size="small" variant="outlined" color="primary" />
      )
    },
    {
      ...
      renderCell: (params) => (
        <Chip
          ...
          color={params.value === 'paid' ? 'success' : params.value === 'cancelled' ? 'error' : 'warning'}
        />
      )
    },
    {
      ...
      renderCell: (params) => (
        <Box sx={{ display: 'flex', gap: 0.5 }}>
          <Tooltip title="View Details">
            ...
            >
              {loadingSale ? <CircularProgress size={16} /> : <Visibility fontSize="small" />}
            </IconButton>
          </Tooltip>
    ...
    <Box>
      ...
      <Grid container spacing={3} mb={3}>
        <Grid item xs={12} sm={4}>
          <Card>
            <CardContent sx={{ display: 'flex', alignItems: 'center', gap: 2, p: 3 }}>
              <Avatar sx={{ width: 52, height: 52, background: 'linear-gradient(135deg, #10b981, #059669)' }}>
                <Receipt />
              </Avatar>
              <Box>
        ...
        <Grid item xs={12} sm={4}>
          <Card>
            <CardContent sx={{ display: 'flex', alignItems: 'center', gap: 2, p: 3 }}>
              <Avatar sx={{ width: 52, height: 52, background: 'linear-gradient(135deg, #2563eb, #7c3aed)' }}>
                <ShoppingCart />
              </Avatar>
              <Box>
                <Typography variant="h4" fontWeight={700}>
