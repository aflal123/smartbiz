...
import {
  ...
  Close, Phone, Email, LocationOn, Business
...
const SupplierDialog = ({ open, onClose, editSupplier }) => {
  ...
  const updateMutation = useMutation({
    mutationFn: (data) => supplierAPI.update(editSupplier.id, data),
    onSuccess: () => {
      queryClient.invalidateQueries(['suppliers']);
  ...
  const handleSubmit = () => {
    ...
    editSupplier ? updateMutation.mutate(formData) : createMutation.mutate(formData);
  };
      ...
      <DialogContent dividers>
        <Grid container spacing={2} sx={{ pt: 1 }}>
          ...
          <Grid item xs={12} sm={6}>
            <TextField
              fullWidth label="Contact Name *" name="name"
...
const SuppliersPage = () => {
  ...
  const { data: suppliers = [], isLoading } = useQuery({
    queryKey: ['suppliers'],
    queryFn:  () => supplierAPI.getAll().then(r => r.data.suppliers),
  });
